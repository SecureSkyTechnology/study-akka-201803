 ...
 */
public class ActorMemoryLeakSolutionDemo implements Runnable {
    ...
    static class Child extends AbstractActor {
        ...
        public Receive createReceive() {
            return receiveBuilder().matchEquals("produce", m -> {
                Random r = new Random();
                r.nextBytes(data);
                // after 1 second, stop my self.
                getContext().system().scheduler().scheduleOnce(FiniteDuration.create(1, TimeUnit.SECONDS), () -> {
                    //getContext().stop(getSelf()); // also works :)
